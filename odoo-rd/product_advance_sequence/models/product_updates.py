...
class ProductCategory(models.Model):
    ...
    def get_created_seq_id(self, product_category_id): 
        parent_category_name = self.env['product.category'].browse(product_category_id).parent_id.name
        category_name = self.env['product.category'].browse(product_category_id).name
        sequence = self.env['ir.sequence']
        created_seq_id = sequence.create({
            'name': 'Code {0}'.format(parent_category_name + ' / ' + category_name),
            'active': True,
...
class Product(models.Model):
    ...
    def get_category_code(self):
        ...
        for product in self:
            ...
            parent_name = category.parent_id.name
            name = category.name
            full_name = parent_name + ' / ' + name
            code = category.code
            seq_id = seq_obj.search([('name', '=', 'Code {0}'.format(full_name))], limit=1)
            if code and seq_id:
                ...
                if product.product_tmpl_id.type in ('product', 'consu') and product.product_tmpl_id.company_id.code:
                    res = {product.id: product.product_tmpl_id.company_id.code + '-' + code + '-' + seq_number}
                else:
        ...
        if values.has_key('categ_id'):
            ...
            if new_sequence:
                ...
                if self.product_tmpl_id.type in ('product', 'consu') and self.product_tmpl_id.company_id.code:
                    new_sequence = self.product_tmpl_id.company_id.code + '-' + category_code + '-' + new_sequence
                else:
