...
class obs_small_label_template(models.AbstractModel):
    ...
    def get_ean_barcode(self, value, width, barWidth = (0.05 * units.inch) /2,
    ...
    def get_code128_barcode(self, value, width, barWidth = (0.05 * units.inch) /2,
                    fontSize = 12, humanReadable = True):
        # El valor por default de fontSize=60
        ...
        for prod in produ_obj.browse(product_ids):
            for product in product_obj.browse(prod.product_id.id):
                ...
                if product.ean13 and data['form']['barcode_from'] == 'ean13':
                    ...
                    barcode_str = self.get_ean_barcode(value = barcode_number, width = 1500)
                elif product.default_code and data['form']['barcode_from'] == 'ref':
                    barcode_number = product.default_code
                    barcode_str = self.get_code128_barcode(value = barcode_number, width = 1500)
                else:
                    continue
