...
class obs_mrp_small_label_template(models.AbstractModel):
    _name = 'report.obs_small_label.obs_mrp_small_label_template'
    ...
    def render_html(self, data=None):
        ...
        report = report_obj._get_report_from_name('obs_small_label.obs_mrp_small_label_template')
        docargs = {
            'doc_ids': self._ids,
    ...
    def _get_barcode_data(self, move_created_ids2):
        #pdb.set_trace()
        print "_get_barcode_data"
        product_list = []
        prod_obj = self.env['product.product']
        mrp_obj = self.env['mrp.production']
        for mrp in mrp_obj.browse(move_created_ids2):
            #if mrp.mrp_type_id and mrp.mrp_type_id.code == 'incoming':
            for line in mrp.move_created_ids2:
                if line.restrict_lot_id:
                    barcode_str = self.get_barcode(value=line.restrict_lot_id.name, width=1500)
                    for qty in range(int(line.product_uom_qty)):
                        prod_dict ={}
                        prod_dict = {
                                'name':line.product_id.name,
                                'default_code':line.restrict_lot_id.name    ,
                                'ean13': barcode_str
                                }
