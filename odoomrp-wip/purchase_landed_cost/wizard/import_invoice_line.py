...
class ImportInvoiceLine(models.TransientModel):
    ...
    def action_import_invoice_line(self):
        ...
        ctx = dict(self._context, lang=self.invoice.partner_id.lang)

        company_currency = self.invoice.company_id.currency_id

        if self.invoice.currency_id != company_currency:
            line_total = 0
            line_currency = self.invoice.currency_id
            line_total = line_currency.compute(self.invoice_line.price_subtotal, company_currency)
        else:
            line_total = self.invoice_line.price_subtotal
